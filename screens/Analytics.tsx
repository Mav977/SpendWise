...
useLayoutEffect(() => {
  navigation.setOptions({
    title: "Analytics",
    headerStyle: {
      backgroundColor: "rgba(115, 0, 255, 0.72)", // Purple background
    },
  ...
  const sortedCategories = categories
    .map((cat) => {
      ...
      return { ...cat, total }; //{id,name,type,total}, total is an addition to the category object
    })
    .sort((a, b) => b.total - a.total);

  
  return (
    <View style={{ flex: 1, paddingTop: 12, backgroundColor: "#F2E7FF" }}>
      <SegmentedControl
      ...
      paddingBottom: 40,
          ...
          renderItem={({ item }) => (
            ...
            <Card style={{marginHorizontal: 10, padding: 15 }}>
               <Text style={{ fontWeight: "bold", fontSize: 16, color: "#7300FF" }}>
          {item.receiver}
        </Text>
